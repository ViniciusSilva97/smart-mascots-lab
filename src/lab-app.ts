...
export const createMascotLab = (app: HTMLDivElement): MascotLab => {
const root = app
const listenerController = new AbortController()
const { signal } = listenerController
let destroyed = false
let targetMarkerFrame: number | null = null
...
app.innerHTML = `
  ...
  <main>
    <section class="intro">
      <p class="eyebrow">// PROTÓTIPO 09 • ROBUSTEZ</p>
...
const progress = app.querySelector<HTMLInputElement>('#progress')!
const progressValue = app.querySelector<HTMLElement>('#progress-value')!
const targetMarker = app.querySelector<HTMLElement>('#target-marker')!
const commandActive = app.querySelector<HTMLElement>('#command-active')!
const commandDecision = app.querySelector<HTMLElement>('#command-decision')!
const commandQueue = app.querySelector<HTMLOListElement>('#command-queue')!
const queueCount = app.querySelector<HTMLElement>('#queue-count')!
const motionMode = app.querySelector<HTMLSelectElement>('#motion-mode')!
const motionModeNote = app.querySelector<HTMLElement>('#motion-mode-note')!
const performancePanel = app.querySelector<HTMLElement>('#performance-panel')!
const performanceQuality = app.querySelector<HTMLElement>('#performance-quality')!
const fpsValue = app.querySelector<HTMLElement>('#fps-value')!
const fpsTarget = app.querySelector<HTMLElement>('#fps-target')!
const frameTime = app.querySelector<HTMLElement>('#frame-time')!
...
const engine = new ByteMotionEngine(byte, {
  onMotionChange(motion) {
    ...
    app.querySelectorAll('.motion-button').forEach(button => {
...
motionMode.addEventListener('change', applyMotionPreference, { signal })
...
app.querySelectorAll<HTMLButtonElement>('.expression-button').forEach(button => {
  button.addEventListener('click', () => {
...
speed.addEventListener('input', () => {
  ...
  app.querySelector<HTMLOutputElement>('#speed-value')!.value = `${Number(speed.value).toFixed(1)}×`
}, { signal })
...
app.querySelectorAll<HTMLButtonElement>('.bg-option').forEach(button => {
  button.addEventListener('click', () => {
    stage.dataset.background = button.dataset.bg
    app.querySelectorAll('.bg-option').forEach(option => option.classList.toggle('active', option === button))
  }, { signal })
})
...
app.querySelector<HTMLButtonElement>('#restart')!.addEventListener('click', () => engine.restart(), { signal })
app.querySelector<HTMLButtonElement>('#demo')!.addEventListener('click', () => {
  orchestrator.dispatch(command({
    kind: 'demo',
    label: 'Demonstração',
    priority: 40,
...
progress.addEventListener('input', () => engine.setProgress(Number(progress.value) / 1000), { signal })

app.querySelector<HTMLButtonElement>('#walk-left')!.addEventListener('click', () => dispatchWalk(-1), { signal })
app.querySelector<HTMLButtonElement>('#walk-right')!.addEventListener('click', () => dispatchWalk(1), { signal })
app.querySelector<HTMLButtonElement>('#run')!.addEventListener('click', () => dispatchWalk(1, true), { signal })
app.querySelector<HTMLButtonElement>('#jump')!.addEventListener('click', () => dispatchJump(), { signal })
app.querySelector<HTMLButtonElement>('#long-jump')!.addEventListener('click', () => dispatchJump(true), { signal })
app.querySelector<HTMLButtonElement>('#clear-queue')!.addEventListener('click', () => orchestrator.clearQueue(), { signal })
app.querySelector<HTMLButtonElement>('#stop-all')!.addEventListener('click', () => {
  speech.textContent = 'Tudo certo. Voltando à posição de atenção.'
  dispatchMotion('idle')
}, { signal })

const trackingButton = app.querySelector<HTMLButtonElement>('#tracking')!
...
app.querySelectorAll<HTMLButtonElement>('.product-target').forEach(target => {
...
stage.addEventListener('pointerleave', () => engine.releaseAttention(), { signal })
...
stage.addEventListener('click', event => {
  ...
  targetMarkerFrame = requestAnimationFrame(() => {
    targetMarkerFrame = null
...
const destroy = () => {
  ...
  root.style.removeProperty('--mascot-scale')
